...
/**
   ...
   */
  MemoryMeshHistory::MemoryMeshHistory(System & system_)
...
MemoryMeshHistory::~MemoryMeshHistory ()
{
}

// This function finds, if it can, the entry where we're supposed to
// be storing data
void MemoryMeshHistory::find_stored_entry(Real time, bool storing)
{
...
// This functions saves the mesh in memory
void MemoryMeshHistory::store(bool /* is_adjoint_solve */, Real time)
{
  ...
  if (stored_meshes.begin() == stored_meshes.end())
    {
      stored_meshes[time] = std::unique_ptr<MeshBase>();
      stored_meshes_it = stored_meshes.begin();
    }
  ...
  // Copy the mesh into the memory
  stored_meshes_it->second = _system.get_mesh().clone();
}

void MemoryMeshHistory::retrieve(bool is_adjoint_solve, Real time)
{
  this->find_stored_entry(time, false);

  // To set the deltat while using adaptive timestepping, we will utilize
  // consecutive time entries in the stored meshes iterator
  ...
  if (stored_meshes_it == stored_meshes.end() ||
...
void MemoryMeshHistory::erase(Real time)
{
