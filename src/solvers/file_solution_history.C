...
{
  // If we have a non-trivial mesh_history, find the corresponding mesh first
  mesh_history->find_stored_entry(time, storing);
...
void FileSolutionHistory::store(bool is_adjoint_solve, Real time)
{
  ...
  if(!is_adjoint_solve)
  {
    ...
    // Write out the current mesh out to file
    std::string mesh_filename = "mesh.out.xda.";
...
void FileSolutionHistory::retrieve(bool is_adjoint_solve, Real time)
{
   ...
   * The problem becomes even more important if the adjoint problem is to
   * be solved on a refined mesh. In this case, we need to avoid losing accuracy
   * on the existing adjoint solution. To do this, we need to clone the existing
   * adjoint. Read in the mesh for the next timestep using mesh history, refine that
   * mesh, and then project the cloned adjoint solution onto the refined mesh.
   * Then save the adjoint solutions, read in the solution history and restore the adjoint solutions.
   */
  if(is_adjoint_solve)
  {
...
{
  // If we have a non-trivial mesh_history, erase the mesh first
  mesh_history->erase(time);
